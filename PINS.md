...
- `0.1.2` (tag `v0.1.2`): same tuple; adds magic literals (`__FILE__`/`__LINE__`/`__ENCODING__`, `1r`/`1i`, backticks, regexps, `:"a#{b}"`, `=~` captures) and fixes the `next`-in-`while`/`until` continue label (issue #2 follow-up).
- `1.2.0` (tag `v1.2.0`): same tuple; adds the real-world corpus tranche (264 files, mruby + picoruby, `carnelian corpus` + baseline gate): `super` splat/block/kwarg arms, circular-arg defaults, MRI numbered blocks, nested-heredoc splits, `<<~` mixed/backslash dedent.
